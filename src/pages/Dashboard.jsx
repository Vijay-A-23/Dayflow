...
export const Dashboard = () => {
  ...
  const todayPunches = attendance.filter(p => p.employeeId === userProfile?.id && p.date === todayStr);
  const activePunch = todayPunches.find(p => p.punchOut === "");
  ...
  // Employee stats
  const employeeAttendance = attendance.filter(a => a.employeeId === userProfile?.id);
  const totalHoursWorked = employeeAttendance.reduce((acc, curr) => acc + curr.totalHours, 0).toFixed(1);
  const latePunches = employeeAttendance.filter(a => a.status === "Late").length;
  
  // Calculate mock leave balance (Total: 20)
  const employeeApprovedLeaves = leaves.filter(l => l.employeeId === userProfile?.id && l.status === "Approved");
  const leavesTaken = employeeApprovedLeaves.reduce((acc, curr) => acc + curr.days, 0);
  const annualBalance = Math.max(12 - employeeApprovedLeaves.filter(l => l.type === "Annual Leave").reduce((acc, curr) => acc + curr.days, 0), 0);
  const sickBalance = Math.max(8 - employeeApprovedLeaves.filter(l => l.type === "Sick Leave").reduce((acc, curr) => acc + curr.days, 0), 0);
    ...
    <div className="space-y-8 p-6 max-w-7xl mx-auto">
      ...
      {userRole !== "admin" && (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          ...
          <div className="glass-card rounded-2xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Annual Leave Balance</p>
              <h3 className="text-2xl font-extrabold text-white">{annualBalance} days</h3>
              <p className="text-[10px] text-slate-400">12 days allocated annually</p>
            </div>
          ...
          <div className="glass-card rounded-2xl p-5 flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-slate-400">8 days allocated annually</p>
            </div>
          ...
          {userRole !== "admin" ? (
            ...
            <div className="glass-panel rounded-2xl p-6 border border-slate-800">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-950/20 p-4 rounded-xl border border-slate-800 hover:border-violet-500/20 transition-all">
                  ...
                  <a href="/employees" className="inline-flex items-center gap-1.5 text-xs text-violet-400 font-semibold hover:text-violet-300">
                ...
                <div className="bg-slate-950/20 p-4 rounded-xl border border-slate-800 hover:border-violet-500/20 transition-all">
                  ...
                  <a href="/leaves" className="inline-flex items-center gap-1.5 text-xs text-violet-400 font-semibold hover:text-violet-300">
